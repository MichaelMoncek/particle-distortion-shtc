...
const folder_name = "results/rectangle_periodic"
...
const dt_frame = max(dt, t_end/250)    #how often data is saved
...
# const FLUID = "tekutina"
# const WALL = "zeď"
# const OBSTACLE = "překážka"
# const GHOST = "bubák"
...
function make_system()
    ...
    obstacle = Rectangle(cyl1 - cyl_r, cyl2 - cyl_r, cyl1 + cyl_r, cyl2 + cyl_r)
    pipe = Rectangle(-inflow_l, 0., chan_l + inflow_l, chan_w)
    wall = BoundaryLayer(pipe, grid, wall_w)
    wall = Specification(wall, x -> (-inflow_l <= x[1] <= chan_l + inflow_l))
...
# # time dependent Poiseulle velocity profile 
# function inflow_velocity(y::Float64, t::Float64)::RealVector
#     s = min(1, t/t_acc)
#     v1 = s*4.0*U_max*y*(chan_w - y)/chan_w^2 #+ s*0.1
#     return v1*VECX
# end
#
# # accelerates particles at inflow to target velocity profile
# function nudge_inflow!(p::Particle, t::Float64)
#     if (p.x[1] < inflow_l) && (p.x[1] > 0) # particles p in 0 < p.x[1] < inflow_l 
#         v_target = inflow_velocity(p.x[2], t)
#         # alpha = 1 at left edge, 0 at right edge of inflow zone
#         # alpha = 1.0 - (p.x[1] + inflow_l) / inflow_l
#         # alpha = 0 at 0, 1 at inflow_l
#         # alpha = p.x[1] / inflow_l 
#         alpha = 1.0
#         # nudge toward target, strength proportional to distance from inlet
#         p.v += alpha * (v_target - p.v) * dt / t_relax
#     end
# end
#
# Poiseulle velocity gradient force
function body_force!(p::Particle)
    if (p.type == FLUID) && (p.x[1] > 0 && p.x[1] < inflow_l)
        # driving pressure gradient dp/dx that produces U_max parabolic profile
        # for Poiseuille flow: dp/dx = -2*mu*U_max*8/chan_w^2
        dpdx = -100*8.0 * mu * U_max / chan_w^2
        p.a += RealVector(-dpdx/p.rho, 0.0, 0.0)
    end
end 
